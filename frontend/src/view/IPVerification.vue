                    ...
                    <div class="space-y-4">
                        ...
                        <div v-if="selectedFile">
                            ...
                            <input type="text" id="file-id" v-model="fileId"
                                ...
                                placeholder="Enter file ID to verify against" />
                        </div>
                    </div>
                    ...
                    <div v-if="verificationResult" class="mt-6">
                        <div :class="verificationResult.isValid ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'"
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <h3 :class="verificationResult.isValid ? 'text-green-800' : 'text-red-800'" class="text-sm font-medium">
                                        {{ verificationResult.isValid ? 'Verified' : 'Verification Failed' }}
                                    </h3>
                                    <div class="mt-2 text-sm" :class="verificationResult.isValid ? 'text-green-700' : 'text-red-700'">
                        ...
                        <!-- IP Details -->
                        <div v-if="verificationResult.ipData" class="mt-4 bg-gray-50 border border-gray-200 rounded-md p-4">
                            ...
                            <dl class="space-y-2 text-sm">
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <dd class="text-gray-900 font-mono text-xs break-all">{{ verificationResult.ipData.documentHash }}</dd>
                                </div>
                            </dl>
                        </div>
                    </div>
...
const { verifyDocumentIntegrity, getIPTimestamp } = useWallet();
...
const verifyDocument = async () => {
    ...
    try {
        ...
        // Verify integrity
        const isValid = await verifyDocumentIntegrity(fileId.value, '0x' + documentHashHex);
